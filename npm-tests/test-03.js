...
function replaceAll(string, search, replace, split) {
  return string.split(search).join(replace).split(split);
}

let items = replaceAll(json.data, ', k', '|k', '|');

let totalItemWithAge32 = 0;

items = items.map((item) => {
  const splitedItem = item.split(', ');

  const preparedObject = {
    key: splitedItem[0].slice(4),
    age: +splitedItem[1].slice(4),
  };

  if (preparedObject.age === 32) {
    totalItemWithAge32++;
  }
...
const cursor = fs.createWriteStream('./output.txt', {});

items.forEach((item) => {
  cursor.write(`${item.key + ' ' + item.age}\n`);
});

console.log('totel itemps with age 32 : ', totalItemWithAge32);
